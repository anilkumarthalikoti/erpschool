...
%>
<body>
	<div class="pageTitle">Fee Structure</div>
		...
		<form name="feestructure" id="feestructure" onSubmit="return false" action="">
			<table width="500" border="0"
				...
				<tr>
					<td>Course</td>
					<td>:</td>
					<td align="right"><select id="batchs" name="batchs" multiple="multiple"
						 selectMultipleMenu>
  ...
  %>
							...
							<option value="<%=course[6]%>" type="radio"
								title="<%=course[3]%>-<%=course[4]%>-<%=course[5]%>"><%=course[3]%>-<%=course[4]%>-<%=course[5]%></option>
							<%}}%>
				...
				<tr>
					...
					<td>:</td>
					<td align="right"><select style="width:350px;" id="financialYears" name="financialYears"
						multiple="multiple"   selectMultipleMenu>
							<%
  ...
  for(AcademicYears academicYear:academicYearDetails){
	  %>
							<option value="<%=academicYear.getId()%>"><%=academicYear.getAcademicYear() %>(<%=sdf.format(academicYear.getAcceptFrom()) %>
								TO
								<%=sdf.format(academicYear.getAcceptTo())%>)							</option>
							<%}%>
					</select></td>
				</tr>
			 <TR style="height:1px;"><TD colspan="3" style="height:2px; background:#000000;"> </TD></TR>
				<tr>
				  <td colspan="3" align="center" style="font-weight:bold;">ENTER AMOUNTS
				   </td>
				</tr>
				<TR style="height:1px;"><TD colspan="3" style="height:2px; background:#000000;"> </TD></TR>
				<tr>
